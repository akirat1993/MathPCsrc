...
\KwIn{A complete bipartite graph $G = (S,T;E)$ with nonnegative cost $c:E \to \mathbb{R}_{\ge 0}$}
\KwOut{The perfect matching $M$ with a minimum total cost}
\tcp{Initialization}
$y(s) \leftarrow 0 \quad \forall s \in Z \cap S$\;
All edges are oriented from $S$ to $T$\;
\While{$ \vert M \vert < \vert S \vert$} {
  \uIf{$R_T \cap Z \neq \emptyset$} {
    \tcp{operation1}
  ...
  \Else{
    \tcp{operation2}
    $\triangle \equiv \min \{ c(i,j) - y(i) - y(j) \mid i \in Z \cap S, j \in T \setminus Z\}$\;
    $y(s) \leftarrow y(s) + \triangle \quad \forall s \in Z \cap S$\;
    $y(t) \leftarrow y(t) - \triangle \quad \forall t \in Z \cap T $\;
  }
}
